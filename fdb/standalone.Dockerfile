FROM internal/vinyl-client

RUN mkdir -p /opt/app/project /opt/app/src
COPY ./vinyl/project/build.properties ./vinyl/project/plugin.sbt ./vinyl/project/scalapb.sbt /opt/app/project/
COPY ./vinyl/build.sbt /opt/app
...
COPY --from=0 /opt/app/target/scala-2.12/scl-assembly-1.0.jar /var/fdb

COPY ./fdb/standalone_start.sh /var/fdb

CMD ./standalone_start.sh 
